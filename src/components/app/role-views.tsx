...
import { formatDate, getAttendanceColor, EmptyState, LoadingCard } from './helpers'
...
export function MySubjectsView() {
  ...
  return (
    <div className="space-y-6">
      <div><h2 className="text-xl font-bold">My Subjects</h2><p className="text-sm text-muted-foreground">{classes.length} enrolled subjects</p></div>
      {classes.length === 0 ? (
        <Card className="shadow-sm"><CardContent><EmptyState icon={BookOpen} title="No subjects" desc="Not enrolled in any subjects yet." /></CardContent></Card>
      ) : (
...
export function MyScheduleView() {
  ...
  return (
    <div className="space-y-6">
      <div><h2 className="text-xl font-bold">My Schedule</h2><p className="text-sm text-muted-foreground">Weekly class schedule</p></div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {WEEKDAYS.map(day => (
...
export function MyAttendanceView() {
  ...
  return (
    <div className="space-y-6">
      <div><h2 className="text-xl font-bold">My Attendance</h2><p className="text-sm text-muted-foreground">{myAttendance.length} total records</p></div>
      ...
      <Card className="shadow-sm">
        <CardContent className="p-0">
          {myAttendance.length === 0 ? (
            <div className="p-8"><EmptyState icon={ClipboardCheck} title="No records" desc="Records will appear here." /></div>
          ) : (
...
export function MyClassesView() {
  ...
  return (
    <div className="space-y-6">
      <div><h2 className="text-xl font-bold">My Classes</h2><p className="text-sm text-muted-foreground">{classes.length} assigned classes</p></div>
      {classes.length === 0 ? (
        <Card className="shadow-sm"><CardContent><EmptyState icon={School} title="No classes" desc="No classes assigned yet." /></CardContent></Card>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {classes.map(c => (
            <Card key={c.id} className="shadow-sm hover:shadow-md transition-shadow">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-3 space-y-1">
                      ...
                      <p className="text-xs text-muted-foreground flex items-center gap-1"><Users className="h-3 w-3" />{c.vacancies} vacancies</p>
                    </div>
                  </div>
                </div>
              </CardContent>
