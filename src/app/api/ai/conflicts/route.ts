        ...
        for (let j = i + 1; j < tSessions.length; j++) {
          ...
          if (date1 === date2 && s1.startTime < s2.endTime && s2.startTime < s1.endTime) {
            conflicts.push({
              ...
              severity: 'CRITICAL',
              confidence: 1.0,
              description: `Teacher "${s1.class.teacher.user.name}" has overlapping sessions on ${date1}`,
        ...
        if (date1 === date2 && s1.roomId && s2.roomId && s1.roomId !== s2.roomId) {
          ...
          if (startMinutes - endMinutes < 15 && startMinutes > endMinutes) {
            conflicts.push({
              ...
              severity: 'LOW',
              confidence: 0.7,
              description: `Teacher "${s1.class.teacher.user.name}" has tight transition between rooms on ${date1} (${s1.endTime} - ${s2.startTime})`,
